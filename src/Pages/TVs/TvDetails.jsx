...
const TvDetails = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let response = await axios.get(`http://localhost:8000/TVs/${id}`);
        setTv(response.data);
  ...
  const updateCart = (newQty) => {
    ...
    if (newQty <= 0) {
      items = items.filter((item) => item.id !== tv.id);
    } else {
      const exists = items.find((item) => item.id === tv.id);
      if (exists) {
        items = items.map((item) =>
          item.id === tv.id ? { ...item, quantity: newQty } : item
        );
      } else {
        items.push({ ...tv, quantity: newQty });
      }
    }
      ...
      <div className="wholediv">
        <div className="container mt-5 details">
          ...
          <div className="description">
            ...
            <p>Rating: ⭐ {tv.rating} / 5</p>
            <h5>Brand: {tv.brand}</h5>
            <h5>Price: ₹{tv.price}</h5>
            ...
            {qty === 0 ? (
              ...
              <>
                ...
                <div className="my-4">
                  <h6>Expected Delivery :- {tv.expectedDelivery}</h6>
                </div>
              </>
            )}
          </div>
        ...
        <div className=" container my-4">
          <h4>User Reviews:</h4>
          {tv.reviews && tv.reviews.length > 0 ? (
            <ul>
              {tv.reviews.map((review, index) => (
                <li key={index} style={{ marginBottom: "1rem" }}>
                  ...
                  <p style={{ marginLeft: 10 }}>{review.comment}</p>
                </li>
              ))}
            </ul>
          ) : (
